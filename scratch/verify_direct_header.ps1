...
Assert-Condition ($css -match 'body:not\(\.auth-locked\)\s*\{[^}]*padding-top:\s*(calc\(var\(--ops-fixed-header-height\)\s*\+\s*var\(--ops-workspace-(top-)?gap\)\)|var\(--ops-header-height\))\s*!important') "body:not(.auth-locked) reserves top space for fixed header" "body padding-top missing"
