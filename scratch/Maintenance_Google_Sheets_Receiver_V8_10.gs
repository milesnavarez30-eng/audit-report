/**
 * CCTV OPS - Maintenance Google Sheets Receiver V8.10
 *
 * STABLE IMAGE INSERTION FIX:
 * - Reverts image insertion to the simple insertImage(blob, column, row) API
 *   that previously rendered correctly on this tracker.
 * - Keeps strict per-block order and screenshot count verification.
 *
 * FIXED BLOCK LAYOUT:
...
function doGet(e) {
  ...
  try {
    ...
    return jsonp_({
      ...
      screenshotMode: 'stable-simple-insert-per-block',
      layout: 'rows > screenshots > remarks',
...
function appendMaintenanceReport_(payload) {
  ...
  try {
    ...
    const imagesBeforeAll = sheet.getImages().length;
    ...
    blocks.forEach(function(block, blockIndex) {
      ...
      const imagesBeforeBlock = sheet.getImages().length;
      ...
      if (screenshots.length) {
        ...
        screenshots.forEach(function(dataUrl, imageIndex) {
          ...
          ensureRowsAvailable_(sheet, imageRow + imageRows);

          // Dedicated "cell area" below the data so images cannot overlap another block.
          sheet.getRange(imageRow, 1, imageRows, 7)
            .setBackground('#ffffff');

          for (let r = imageRow; r < imageRow + imageRows; r++) {
          ...
          // IMPORTANT V8.10:
          // Use the simple 3-argument insertImage(blob, column, row) call.
          // This is the same insertion style that rendered correctly in the earlier working receiver.
          // We avoid pixel-offset insertion because it produced blank proof areas on this tracker.
          const image = sheet.insertImage(blob, 3, imageRow);
          image.setWidth(350);
          image.setHeight(210);
          ...
          try {
            ...
            image.setAltTextDescription(
              'Maintenance block ' +
              blockNumber +
              ' screenshot. Anchor row ' +
              imageRow
            );
          } catch (altTextError) {
            console.log('Alt text skipped: ' + altTextError);
          }

          SpreadsheetApp.flush();

          // Verify the new over-grid image is anchored where this block expects it.
          const currentImages = sheet.getImages();
          const inserted = currentImages[currentImages.length - 1];
          if (!inserted) {
            throw new Error(
              'Block ' + blockNumber +
              ' screenshot ' + (imageIndex + 1) +
              ' was not created in Google Sheets.'
            );
          }

          try {
            const anchor = inserted.getAnchorCell();
            if (anchor && anchor.getRow() !== imageRow) {
              throw new Error(
                'Block ' + blockNumber +
                ' screenshot ' + (imageIndex + 1) +
                ' anchored at row ' + anchor.getRow() +
                ' instead of row ' + imageRow + '.'
              );
            }
          } catch (anchorError) {
            // If the runtime cannot expose anchor metadata, do not hide an actual insert.
            console.log('Anchor verification note: ' + anchorError);
          }
      ...
      // Verify THIS BLOCK before writing remarks / next block.
      SpreadsheetApp.flush();

      const imagesAfterBlock = sheet.getImages().length;
      const actualBlockImages = Math.max(0, imagesAfterBlock - imagesBeforeBlock);
    ...
    const imagesAfterAll = sheet.getImages().length;
    const actualAllImages = Math.max(0, imagesAfterAll - imagesBeforeAll);

    if (actualAllImages !== totalExpectedImages) {
      throw new Error(
    ...
    return {
      ...
      insertedImageCount: actualAllImages,
